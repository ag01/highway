...
HWY_NOINLINE bool MaybePartitionTwoValueR(D d, Traits st, T* HWY_RESTRICT keys,
  ...
  HWY_DASSERT(num >= N);
  size_t i = num;       // END of vector load
  size_t writeR = num;  // start of LAST written

  // As long as all lanes are equal to L or R, we can overwrite with valueR.
  // This is faster than first counting, then backtracking to fill L and R.
  for (; i >= N; i -= N) {
    const size_t read = i - N;
    const Vec<D> v = LoadU(d, keys + read);
    // It is not clear how to apply OrXor here - that can check if *both*
    // comparisons are true, but here we want *either*. Comparing the unsigned
    // min of differences to zero works, but is expensive for u64 prior to AVX3.
    const Mask<D> eqL = st.EqualKeys(d, v, valueL);
    ...
    if (HWY_UNLIKELY(AllTrue(d, Or(eqL, eqR)))) {
      ...
      HWY_DASSERT(lane >= 0);
      third = st.SetKey(d, keys + read + static_cast<size_t>(lane));
      if (VQSORT_PRINT >= 2) {
        fprintf(stderr, "found 3rd value at vec %zu; writeR %zu\n", read,
                writeR);
        MaybePrintVector(d, "third", third, 0, st.LanesPerKey());
      }
      // 'Undo' what we did by filling the remainder of what we read with L.
      // For clarity, we fill in ascending order.
      size_t pos = i;
      for (; pos + N <= writeR; pos += N) {
        StoreU(valueL, d, keys + pos);
      }
      BlendedStore(valueL, FirstN(d, writeR - pos), d, keys + pos);
      return false;
    }
    HWY_DASSERT(writeR >= N);
    StoreU(valueR, d, keys + writeR - N);
    writeR -= CountTrue(d, eqR);
  }

  // Final vector, masked comparison (no effect if i == 0)
  // No need for SafeCopyN because we checked there is at least one vector.
  const size_t remaining = i;
  ...
  if (HWY_UNLIKELY(!AllTrue(d, eq))) {
    ...
    // 'Undo' what we did by filling the remainder of what we read with L.
    BlendedStore(valueL, FirstN(d, writeR), d, keys);
    return false;
  }
  writeR -= CountTrue(d, eqR);
  BlendedStore(valueR, valid, d, keys + writeR);
  ...
  if (VQSORT_PRINT >= 2) {
    fprintf(stderr, "Successful MaybePartitionTwoValueR\n");
  }
  return true;
}
...
// value, and sets idx_third to the position of one of them. Otherwise,
// partitions the array and returns true.
template <class D, class Traits, typename T>
HWY_INLINE bool PartitionIfTwoKeys(D d, Traits st, const Vec<D> pivot,
  ...
  return is_pivotR
             ? MaybePartitionTwoValueR(d, st, keys, num, pivot, second, third)
             : MaybePartitionTwoValue(d, st, keys + idx_second,
                                      num - idx_second, pivot, second, third,
                                      buf);
}
...
template <class D, class Traits, typename T>
HWY_NOINLINE bool FindAnyBefore(D d, Traits st, const T* HWY_RESTRICT keys,
                                size_t num, const Vec<D> pivot) {
  const size_t N = Lanes(d);
  HWY_DASSERT(num >= N);  // See HandleSpecialCases
...
template <class D, class Traits, typename T>
HWY_NOINLINE bool FindAnyAfter(D d, Traits st, const T* HWY_RESTRICT keys,
                               size_t num, const Vec<D> pivot) {
  const size_t N = Lanes(d);
  HWY_DASSERT(num >= N);  // See HandleSpecialCases
...
HWY_INLINE Vec<D> ChoosePivotForEqualSamples(D d, Traits st,
  ...
  if (HWY_UNLIKELY(before)) {
    ...
    if (HWY_UNLIKELY(after || FindAnyAfter(d, st, keys, num, pivot))) {
  ...
  if (HWY_UNLIKELY(FindAnyBefore(d, st, keys, num, pivot))) {
    result = PivotResult::kNormal;
    return pivot;
  }
